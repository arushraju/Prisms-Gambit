...
typedef struct board_evaluation
{
    int evaluation; //This will store the evaluation data
    struct board_evaluation* next;//This will point to the next node
}board_eval;
//This will store the evalution of the board.
board_eval* score = NULL;

//And these functions are to be used for pushing and pulling the data from board_evaluation
void eval_push(int data){
    if(score == NULL){
        board_eval *p = (board_eval*)malloc(sizeof(board_eval));
        p->evaluation = data;
        p->next = NULL;
        score = p;
        return;
    } else {
        //Make a board_eval pointer that will point to the last node of the score.
        board_eval* q = score;
        //Traverse this pointer to the last node
        while(q->next != NULL){
            q = q->next;
        }
        //Then we make a new board_eval* and add this to the q->next;
        board_eval *p = (board_eval*)malloc(sizeof(board_eval));
        p->evaluation = data;
        p->next = NULL;
        //And then attach this as the last node
        q->next = p;
        return;
    }
}
//This structure will store the interger (which is sum of all the evaluation), and also the number of integer in the linked list
struct score_sum_num{
    int sum;
    int num;
};

//And this will be used to calculate the sum of all the elements in the score linked list
struct score_sum_num sum_score(){
    struct score_sum_num answer;
    int sum = 0;
    int num = 0;
    board_eval* p = score;
    while(p!=NULL){
        num++;
        sum = sum + p->evaluation;
        p = p->next;
    }
    answer.num = num;
    answer.sum = sum;
...
//This will calculate the evlaution from the evlaution linked list finally
double board_evaluation(){
    struct score_sum_num sum_num;
    sum_num = sum_score();
    
    int num = sum_num.num;
    //printf("The number of elements in the score array is %d\n",num);
    int sum = sum_num.sum;
    //printf("The sum of all the elements in the array %d\n",sum);

    //This will return the average
    //printf("Average of the sum and num is %d",(int)sum/num);
    if(num) return (double)sum/num; 
    else{return 0;}
...
void assign_empty(struct peice* board_dash,int _position_)
{
    board_dash[_position_].position = _position_;
    board_dash[_position_].orientation = 0;
    board_dash[_position_].peice_color = 'n';
    board_dash[_position_].peice_type = 'n';
    board_dash[_position_].points = 0;
}
...
bool Is_move_possible(struct peice* board_dash,char which_peice,int present_position, int direction)
{
    ...
    //1.1)This will not let the colors of opposite color to come in front of laser whose colors are of it's opposite color
    
    //If the piece is red and the position where it wants to go is infront of blue's laser then that's not allowed.
    if(board_dash[present_position].peice_color == 'r'){
        if(moved_position>=1 && moved_position <= 8){
            if(output_print)fprintf(output_file,"Attempt to move Red piece in front of Blue\n");
            return  false;
        }
    }
    //if the piece is Blue and the position where it wnats to go is in front of blue's laser then thats not allowed
    if(board_dash[present_position].peice_color == 'b'){
        if(moved_position>=73 && moved_position <= 80){
            if(output_print)fprintf(output_file,"Attempt to move Blue piece in front of Red\n");
            return  false;
        }
    }
...
//This function will take the array of two boards, one that needs to be copied and then copied board itself. And it will update them. it won't return anaything
void copy_board(struct peice* copied_board,struct peice* current_board)
{
    //I am skippijng the 0th index.
    for(int i = 1;i<81;i++)
    {
...
struct tree_node minimax(struct peice* current_board,int depth,bool maximizer_blue,int alpha,int beta,bool is_root_call)
{
    ...
    struct peice child_board[81];    //This pointer will store the current board, and will be used for iterations.
    struct tree_node child;          //This pointer will store the data from recursion (child board and its evaluation) 
    struct peice best_child[81];     //This will store the best child for the parent.

    //Just in case 
    copy_board(best_child, current_board);
    ...
    if(maximizer_blue)
    {
        ...
        {   
            //So this is blue's turn and here we have returned the copy of the curretn board along with it's evaluation after the red laser is fired.
            struct tree_node node;
            
            if(is_root_call) {
                copy_board(node.data,current_board);// eval_push(node.evaluation);
                //printf("Evaluation = %d\n",node.evaluation);
            }
            ...
            //I will add the evalution at the leaf
            //printf("Evaluation = %d\n",node.evaluation);

            //Return the copy of this node. Safer.
            return node;
        }
        
        

        if(output_print)fprintf(output_file,"So Blue is not loosing after Red laser is fired\n");
        ...
        while(p!=NULL)
        {
            ...
            if(Is_move_possible(current_board,p->data.peice_type,p->data.position,+1))
            {
                ...
                copy_board(child_board,current_board);
                ...
                if(child.evaluation>maxEval)
                {
                    ...
                    if(is_root_call) {copy_board(best_child,child_board); eval_push(maxEval); /*printf("Evaluation = %d\n",maxEval);*/}
                }
                alpha = max(alpha,child.evaluation);
                if(beta<=alpha)
                {
                    if(output_print)fprintf(output_file,"Puring occured\n");
                    break;
                }
            }
            ...
            if(output_print)fprintf(output_file,"\n");
            if(output_print)fprintf(output_file,"Now lets move to the next child\n");
        ...
        struct tree_node node;
        copy_board(node.data,best_child);
    ...
    {
        //Before red plays the game once, Blue lazer must be fired
        ...
        while(p!=NULL)
        {
            ...
            if(Is_move_possible(current_board,p->data.peice_type,p->data.position,+1))
            {
                ...
                if(minEval>child.evaluation)
                {

                    minEval = child.evaluation;
                    if(is_root_call) {copy_board(best_child,child_board); eval_push(minEval); /*printf("Evaluation = %d\n",maxEval);*/}
...
void decode(int* array,peice* board){
    for(int i=0;i<320;){
...
int main(int argc, char* argv[])
{
    if(specific_test){
        ...
        int decoded_array[321];

        for (int i = 0; i < 320; i++) {
            encoded_array[i] = atoi(argv[i + 2]);
        }

        decode(encoded_array, board);
        printf("Before Board : \n");
        Display(board);

        best_move = minimax(board, 5, false, -10000, 10000, true);

        copy_board(board,best_move.data);
        ...
        //And finally I will add the evaluation here into the decode string and will print it
        decoded_array[320] = (int) board_evaluation();
        printf("\n");
        for (int i = 0; i < 320; i++) {
        ...
        /**The first argument is the string "./solver". Second argument is the depth of the engine, and finally the rest which contains 320 elements will be the state of board. */
        if (argc < 322) {
            printf("No input detected\n");
            return 1;
        }
        ...
        //This should return the board as it is.
